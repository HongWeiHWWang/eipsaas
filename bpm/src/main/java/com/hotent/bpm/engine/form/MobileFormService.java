...
public class MobileFormService extends AbstractFormService {
	...
	public FormModel getInstanceNodeForm(BpmProcessInstance instance, String defId,
		...
		// 获取实例的节点表单
		BpmInstForm bpmInstForm = bpmInstFormManager.getNodeForm(instance.getId(), defId, nodeId,FormType.MOBILE.value());
		...
		if(isSub){
			if(isInstForm){
				...
				bpmInstForm = bpmInstFormManager.getNodeForm(instance.getId(), defId, instance.getId(),FormType.MOBILE.value());
			}else{
				...
				if(BeanUtils.isEmpty(form)){
					...
					for (BpmNodeDef nodeDef : nodeDefs) {
						if(nodeDef.getType().equals(NodeType.START)&&BeanUtils.isNotEmpty(nodeDef.getSubFormList())){
							bpmInstForm = bpmInstFormManager.getNodeForm(instance.getId(), instance.getProcDefId(), nodeDef.getNodeId(),FormType.MOBILE.value());
							break;
						}
					}
					//获取主流程第一个节点定义表单
					if(BeanUtils.isEmpty(bpmInstForm)){
						...
						if(BeanUtils.isNotEmpty(startNodes)){
							bpmInstForm = bpmInstFormManager.getNodeForm(instance.getId(), instance.getProcDefId(), startNodes.get(0).getNodeId(),FormType.MOBILE.value());
						}
					}
				}
			}
		...
		// 全局表单
		if(BeanUtils.isEmpty(bpmInstForm)){
			bpmInstForm = bpmInstFormManager.getGlobalForm(instance.getId(),FormType.MOBILE.value());
		}
