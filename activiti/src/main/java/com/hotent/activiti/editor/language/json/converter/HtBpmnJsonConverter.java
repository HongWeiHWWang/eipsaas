 ...
 */
public class HtBpmnJsonConverter implements EditorJsonConstants, StencilConstants, ActivityProcessor,HtStencilConstants {
    ...
    static {
        ...
        //hotent add 
        HtStartEventJsonConverter.fillTypes(convertersToBpmnMap,
				convertersToJsonMap);
		HtSignTaskJsonConverter.fillTypes(convertersToBpmnMap,
				convertersToJsonMap);
		
		HtCustomSignTaskJsonConverter.fillTypes(convertersToBpmnMap,
				convertersToJsonMap);
		
		HtMessageTaskJsonConverter.fillTypes(convertersToBpmnMap,
				convertersToJsonMap);
		HtEndEventJsonConverter.fillTypes(convertersToBpmnMap,
				convertersToJsonMap);
		HtStartSubFlowTaskJsonConverter.fillTypes(convertersToBpmnMap,
				convertersToJsonMap);
		HtAndGatewayJsonConverter.fillTypes(convertersToBpmnMap,
				convertersToJsonMap);
		HtXorGatewayJsonConverter.fillTypes(convertersToBpmnMap,
				convertersToJsonMap);
		HtOrGatewayJsonConverter.fillTypes(convertersToBpmnMap,
				convertersToJsonMap);
		HtScriptTaskJsonConverter.fillTypes(convertersToBpmnMap,
				convertersToJsonMap);
    }
    ...
    static {
        ...
        //hotent add
        DI_RECTANGLES.add(STENCIL_TASK_SIGN);
        DI_RECTANGLES.add(STENCIL_TASK_CUSTOME_SIGN);
        DI_RECTANGLES.add(STENCIL_TASK_MESSAGE);
        DI_RECTANGLES.add(STENCIL_TASK_STARTSUBTASK);
        DI_GATEWAY.add(STENCIL_XORGATEWAY_EVENT);
        DI_GATEWAY.add(STENCIL_ANDGATEWAY_EVENT);
        DI_GATEWAY.add(STENCIL_ORGATEWAY_EVENT);
        DI_CIRCLES.add(STENCIL_EVENT_START_DEFAULT);
        DI_CIRCLES.add(STENCIL_EVENT_END_DEFAULT);
    ...
    public BpmnModel convertToBpmnModel(JsonNode modelNode) {
        ...
        for (JsonNode shapeNode : shapesArrayNode) {
            ...
            if (STENCIL_POOL.equals(stencilId)) {
                ...
                pool.setProcessRef(BpmnJsonConverterUtil.getPropertyValueAsString(PROPERTY_PROCESS_ID, modelNode));
        ...
        if (nonEmptyPoolFound == false) {
            ...
            if (BeanUtils.isNotEmpty(bpmnModel.getProcesses())) {
            	BpmnJsonConverterUtil.convertJsonToListeners(modelNode, bpmnModel.getProcesses().get(0));
			}
        }
