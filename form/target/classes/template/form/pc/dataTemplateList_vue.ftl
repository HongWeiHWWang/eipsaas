...
<#macro genCondition field>
	...
	<el-form-item label="${field.cm}" style="padding-right:10px;">
	<#if field.vf=="static" >
		<#switch field.ct>
			<#case "onetext">
				<#if field.ty == 'number'>
				...
				<#else>
					<el-input type="text" size="mini" class="search-item" ht-query="${colPrefix}${field.na}" v-model="searchForm.${colPrefix}${field.na}" operation="${field.qt}" placeholder="${field.cm}" name="Q^${colPrefix}${field.na}^${field.qt}" clearable></el-input>
				</#if>
			<#break>
			<#-- 自定义对话框 -->
			<#case "customDialog">
...
<#macro genManage manage managePermission actionUrl data>
	...
	<#if manage.name == 'edit'>
		<#if managePermission.edit>
			<el-button v-if="permission.edit" type="text" @click='operating("${templateId}",scope.row.${pkField},"edit","","","${manage.openType}");' size="mini" plain>${manage.desc}</el-button>
		</#if>
    <#--打印-->
    <#elseif manage.name == 'printDetail'>
        <#if managePermission.printDetail>
            <el-button type="text" size="mini" @click='printDetail("${templateId}",scope.row.${pkField},"get")'>${manage.desc}</el-button>
        </#if>
	<#--删除-->
	<#elseif manage.name == 'del' >
		<#if managePermission.del>
			<el-button type="text" v-if="permission.del" @click='del(scope.row.${pkField});' size="mini" plain>${manage.desc}</el-button>
		</#if>

	<#--子表-->
	<#elseif manage.name == 'sub'>
		<#if managePermission.sub && hasSub>
			<el-button type="text" @click='showSubList(scope.row.${pkField});' size="mini" plain>${manage.desc}</el-button>
		</#if>
	<#--生成二维码-->
	<#elseif manage.name == 'produceQRCode' >
		<#if managePermission.produceQRCode>
			<el-button type="text" v-if='<#if bpmDataTemplate.mobileFormAlias?exists>true<#else>false</#if>  ' @click='getQRCode("${templateId}",scope.row.${pkField},"${bpmDataTemplate.mobileFormAlias}",${JsonUtil.toJson(manage.display)},scope.$index)' size="mini" plain>${manage.desc}</el-button>
	</#if>
	<#--明细-->
	<#elseif manage.name == 'detail' >
		<#if managePermission.detail>
			<el-button type="text" @click='operating("${templateId}",scope.row.${pkField},"get","","","${manage.openType}");' size="mini" plain>${manage.desc}</el-button>
		</#if>
	<#elseif manage.name == 'url' &&manage.type=='1'>
		<#if managePermission.url>
		<el-button type="text" @click="openUrl('${manage.url}','new','','')" size="mini" plain>${manage.desc}</el-button>
		</#if>
	<#--表单修改记录-->
	<#elseif manage.name == 'record' >
		<#if managePermission.record>
			<el-button type="text" @click="openRecordList(scope.row.${pkField})" size="mini" plain>${manage.desc}</el-button>
		</#if>
	<#elseif manage.name == 'switch' >
		<#if managePermission.switch>
			<#assign json = JsonUtil.toJson(manage.display)?eval />
			<#list json as display>
				<#assign item = display?eval/>
					<el-tooltip content="${manage.desc}" effect="dark">
						<el-switch
							@change='switchChange("${manage.switchOn}","${manage.switchOff}",${JsonUtil.toJson(manage.display)},scope.row,"${manage.switchDefaultTrue}")'
							v-model='switchStatus[rowIndex(scope.row)]'
							>
						</el-switch>
					</el-tooltip>
			</#list>
...
<#macro genToolBar manage managePermission actionUrl>
    ...
    <#if manage.name == 'print'>
        <#if managePermission.print>
            <el-button type="primary" v-if="permission.print" size="mini" @click="printList()" icon="el-icon-printer">${manage.desc}</el-button>
        </#if>
    </#if>
	<#-- 新增 -->
	<#if manage.name == 'add'>
		<#if managePermission.add>
		   <el-button type="primary" v-if="permission.add" size="mini" @click="operating('${templateId}','', 'add','${bpmDataTemplate.defId}','','${manage.openType}')" icon="el-icon-plus">${manage.desc}</el-button>
		</#if>
	</#if>
	<#-- 生成二维码 -->
	<#if manage.name == 'produceQRCode'>
		<#if managePermission.produceQRCode>
			<el-button  v-if='<#if bpmDataTemplate.mobileFormAlias?exists>true<#else>false</#if>  ' @click='downloadAllQRCode("${templateId}","${bpmDataTemplate.mobileFormAlias}",${JsonUtil.toJson(manage.display)},"${pkField}")' size="mini" plain icon="el-icon-s-grid">${manage.desc}</el-button>
		</#if>
	</#if>
	<#-- 启动流程 -->
	...
	<#if manage.name == 'del'>
		<#if managePermission.del>
		   <el-button type="danger" v-if="permission.del" size="mini" @click="deleted()" icon="el-icon-delete">${manage.desc}</el-button>
		</#if>
	</#if>
	...
	<#if manage.name == 'export'>
		<#if managePermission.export>
		    <el-button type="warning" v-if="permission.export" size="mini" @click="exports('${actionUrl.export}')" icon="el-icon-download">${manage.desc}</el-button>
	...
	<#if manage.name == 'import'>
		<#if managePermission.import>
			<el-dropdown split-button v-if="permission.add" size="mini" type="warning" @command="importCommand">
			  <el-upload
				action=""
				:show-file-list="false"
				:http-request="importMain"
				accept=".xls,.xlsx">
				<span> <i class="el-icon-upload2"></i>${manage.desc}</span>
			</el-upload>
			  <el-dropdown-menu slot="dropdown">
			    <el-dropdown-item :command="{command:'downloadTempFile'}">下载模板</el-dropdown-item>
			    <el-dropdown-item v-if="dataView && dataView.fillValue" :command="{command:'log'}">查看异常日志</el-dropdown-item>
			  </el-dropdown-menu>
			</el-dropdown>
		</#if>
	</#if>
	...
	<#if managePermission.url>
		<el-button type="info" size="mini" @click="openUrl('${manage.url}','new','','')" icon="el-icon-share">${manage.desc}</el-button>
		...
		<el-container>
			<el-main>
				<el-row>
					...
					<el-dialog title="查看子表数据" :visible.sync="dialogSubVisible" width="750px" id="subDialogs" :before-close="subCancel">
						<el-tabs type="border-card" v-model="currentTabName">
							<el-tab-pane v-for="tab in tabs" :label="tab.comment" :key="tab.id">
								<el-container>
									<el-main style="padding:0px;">
										<el-input placeholder="请输入内容" v-model="querySubValue" clearable>
										...
										<el-upload
												...
												:show-file-list="false"
												accept=".xls,.xlsx">
											<el-button @click="importButton(tab)">导入</el-button>
										</el-upload>
										...
										<el-table
											...
											<el-table-column v-for="attributes in tab.attributeList" :key="attributes.id"
					...
					<#if isIndistinct == 'show'>
						<el-row>
							<el-form :inline="true" :model="queryForm" class="demo-form-inline">
								...
								<el-col :span="6">
									<el-button size="mini" style="margin-left: 20px" type="primary" icon="el-icon-search" @click="search('find')">查询</el-button>
									<el-button size="mini"  icon="el-icon-refresh" @click="reset">重置</el-button>
									<el-button size="mini"  @click="advanced()" :icon="incons" type="info"></el-button>
								</el-col>
							</el-form>
						</el-row>
					<#--高级检索区域-->
					...
					<#if isIndistinct == 'hide'>
						<el-row>
							<el-form :inline="true" :model="searchForm" class="demo-form-inline">
								...
								<el-form-item>
									<el-button size="mini" type="primary" icon="el-icon-search" @click="search('find')">查询</el-button>
									<el-button size="mini" icon="el-icon-refresh" @click="reset">重置</el-button>
								</el-form-item>
							</el-form>
						</el-row>
                ...
                <span id="printData">
                    <el-table
                        ...
                        <el-table-column prop="${field.name}" column-key="${field.name}" align="center" label="${field.desc}" <#if util.getJsonByPath(filteringFieldMap,fieldName) != "" >:filters='${util.getJsonByPath(filteringFieldMap,fieldName)}'</#if> style="width: 15%" sortable show-overflow-tooltip>
								<template slot-scope="scope">
                    ...
                    <el-table-column
                        ...
                        <template slot-scope="scope">
                            <#noparse>
		                        ...
		                        <el-button size="mini" type="text" v-if='isJoinFlow=="true"&&taskType=="todo"' @click='complete(scope.row.bpm_proc_inst_id_);'  plain>处理任务</el-button>
		                    </#noparse>
                    	</template>
                    </el-table-column>
                    <#noparse>
